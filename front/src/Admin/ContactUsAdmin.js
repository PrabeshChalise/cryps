...
const ContactUsAdmin = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        // Fetch contacts for the clients of the logged-in admin's agents
        const response = await axios.get(
          `http://localhost:3001/api/contacts-for-admin/${adminId}`
        );
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(
        `http://localhost:3001/api/contacts/${selectedContactId}`
      );
      setContacts(
