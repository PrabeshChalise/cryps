...
app.get("/api/wallet/:userId/total-balance", async (req, res) => {
  try {
    ...
    // Get all balances except USD
    const balances = wallet.balances;
    const symbols = Object.keys(balances).filter((symbol) => symbol !== "usd");
    ...
    symbols.forEach((symbol) => {
      const usdPrice = prices[symbol]?.usd || 0;
      totalUsdBalance += balances[symbol] * usdPrice;
    });
...
app.post("/api/send", async (req, res) => {
  ...
  try {
    const wallet = await Wallet.findOne({ userId });
    if (!wallet || wallet.balances[symbol] < amount) {
      return res.status(400).json({ error: "Insufficient balance" });
    }

    // Create and save the send request
    const sendRequest = new Send({
      userId,
      symbol,
      amount,
    ...
    // Check if a frozen amount entry already exists for this user and symbol
    let frozenAmount = await FrozenAmount.findOne({ userId, symbol });

    if (frozenAmount) {
      // If entry exists, add the amount to the existing frozen amount
      frozenAmount.amount += amount;
    } else {
      // Otherwise, create a new frozen amount entry
    ...
    // Save the frozen amount
    await frozenAmount.save();

    res.json({
      success: true,
      message: "Send request submitted, balance deducted, and amount frozen.",
    });
  } catch (error) {
    console.error("Error creating send request:", error);
    res.status(500).json({ error: error.message });
